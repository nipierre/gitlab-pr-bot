...
async fn main() -> Result<(), Error> {
    ...
    let gitlabs: Vec<GitLab> = if matches.occurrences_of("vault") == 1 {
        ...
        dotenv().ok();
        let urls: Vec<String> = env::var("GITLAB_URL")
            .unwrap()
            .split(':')
            .map(String::from)
            .collect();
        let mut tokens: Vec<String> = env::var("GITLAB_TOKEN")
            .unwrap()
            .split(':')
            .map(String::from)
            .collect();
        ...
        vec![GitLab { url, token }]
    };

    let required_reviewer = matches
        .value_of("number-reviewers")
        .unwrap()
        .parse::<usize>()
    ...
    for gitlab in gitlabs {
        ...
        for repo in repos {
            let repo_url_pr = format!(
                "https://{gitlab}/api/v4/projects/{repo_id}/merge_requests?",
                gitlab = gitlab.url,
                repo_id = repo.id
            );
...
mod test {
    ...
    use teams::{extract_project_name, get_assignees, Card};
    ...
    fn test_get_assignees() {
        let mock_assignees: Vec<Author> = vec![Author {
            id: 12345,
            name: "Toto Tata".into(),
    ...
    fn test_card_creation() {
        ...
        assert_eq!(
            card.sections[0].activityTitle,
            format!("Toto Tata (toto123) needs reviewer(s) for this PR : *Title*")
        );
